...
describe("testing index.js routes", () => {
  ...
  it("POST /profile - update the profile", async () => {
    const res = await request
      .post("/profile")
      .set("Cookie", [session_info])
      .send({ firstname: "Howard", lastname: "Rogers", dob: "1923/09/19", education: "", career: ""})
    console.log(res.text);
    expect(res.statusCode).toEqual(200);
    expect(res.text).toEqual("Profile Updated.");
  });

  it("GET /profile - updated profile information", async () => {
    const res = await request
      .get("/profile")
      .set("Cookie", [session_info])
    console.log(res.text);
