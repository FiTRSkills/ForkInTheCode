...
var devDependencies = {
	...
	"eslint-plugin-prettier-internal-rules":
		"file:scripts/tools/eslint-plugin-prettier-internal-rules",
...
var scripts = {
	...
	"lint:dist":
		'eslint --no-eslintrc --no-ignore --env=es6,browser --parser-options=ecmaVersion:2016 "dist/!(bin-prettier|index|third-party).js"',
...
const { isArray } = Array; // `supportsColor.level` → `ansiStyles.color[name]` mapping
...
function baseIsEqualDeep(object, other, bitmask, customizer, equalFunc, stack) {
	...
	if (!(bitmask & COMPARE_PARTIAL_FLAG$3)) {
		var objIsWrapped =
			...
			othIsWrapped =
				othIsObj && hasOwnProperty$9.call(other, "__wrapped__");
...
var partition = _createAggregator(
...
var minimist_1 = function (args, options) {
	...
	const newDefaults = Object.assign(
		{},
		defaults,
		fromPairs_1(booleanWithoutDefault.map((key) => [key, PLACEHOLDER]))
	);
...
function filterPatterns(arr, comparison, prefix, intersection, options) {
	...
	for (let ele of arr) {
		let { string } = ele; // only push if _both_ are negative...
...
const fillNumbers = (start, end, step = 1, options = {}) => {
	...
	let maxLen = padded
		...
		: 0;
	let toNumber =
		padded === false && stringify$1(start, end, options) === false;
...
const fillLetters = (start, end, step = 1, options = {}) => {
	if (
		(!isNumber$2(start) && start.length > 1) ||
		(!isNumber$2(end) && end.length > 1)
	) {
...
micromatch.isMatch = (str, patterns, options) =>
	picomatch$1(patterns, options)(str);
/**
 * Backwards compatibility
 */
...
var tasks = createCommonjsModule(function (module, exports) {
	...
	function convertPatternsToTasks(positive, negative, dynamic) {
		...
		if ("." in positivePatternsGroup) {
			const task = convertPatternGroupToTask(
				".",
				positive,
				negative,
				dynamic
		...
		return convertPatternGroupsToTasks(
			positivePatternsGroup,
			negative,
			dynamic
		);
	}
	...
	function makeRplTaskEntry(entry, settings) {
		return (done) => {
			...
			settings.fs.stat(entry.path, (statError, stats) => {
				...
				entry.dirent = utils$3.fs.createDirentFromStats(
					entry.name,
					stats
				);
				return done(null, entry);
	...
	function readdir(directory, settings, callback) {
		settings.fs.readdir(directory, (readdirError, names) => {
			...
			const tasks = filepaths.map((filepath) => {
				return (done) =>
					out.stat(filepath, settings.fsStatSettings, done);
			});
	...
	class Matcher {
		...
		_getPatternSegments(pattern) {
			...
			return parts.map((part) => {
				const dynamic = utils$2.pattern.isDynamicPattern(
					part,
					this._settings
				);
				...
				return {
					dynamic: true,
					pattern: part,
					patternRe: utils$2.pattern.makeRe(
						part,
						this._micromatchOptions
					),
				};
		...
		_splitSegmentsIntoSections(segments) {
			return utils$2.array.splitWhen(
				...
				(segment) =>
					segment.dynamic &&
					utils$2.pattern.hasGlobStar(segment.pattern)
			);
		}
	}
...
var entry = createCommonjsModule(function (module, exports) {
	...
	class EntryFilter {
		...
		_filter(entry, positiveRe, negativeRe) {
			...
			if (
				this._isSkippedByAbsoluteNegativePatterns(
					entry.path,
					negativeRe
				)
			) {
		...
		_onlyDirectoryFilter(entry) {
			return (
				this._settings.onlyDirectories && !entry.dirent.isDirectory()
			);
		}
		...
		_isSkippedByAbsoluteNegativePatterns(entryPath, patternsRe) {
			...
			const fullpath = utils$2.path.makeAbsolute(
				this._settings.cwd,
				entryPath
			);
...
var entry$1 = createCommonjsModule(function (module, exports) {
	...
	class EntryTransformer {
		...
		_transform(entry) {
			...
			if (this._settings.absolute) {
				filepath = utils$2.path.makeAbsolute(
					this._settings.cwd,
					filepath
				);
		...
		read(task) {
			...
			return new Promise((resolve, reject) => {
				...
				stream.on("data", (entry) =>
					entries.push(options.transform(entry))
				);
...
function* expandPatternsInternal(context) {
	...
	for (const pattern of context.filePatterns) {
		...
		if (
			containsIgnoredPathSegment(absolutePath, cwd, silentlyIgnoredDirs)
		) {
			continue;
		}
...
const { isCI } = thirdParty; // Some CI pipelines incorrectly report process.stdout.isTTY status,
...
function logResolvedConfigPathOrDie(context) {
	const configFile = src.resolveConfigFile.sync(
		context.argv["find-config-path"]
	);

	if (configFile) {
		context.logger.log(
			path__default["default"].relative(process.cwd(), configFile)
		);
	} else {
		process.exit(1);
	}
	...
	context.logger.log(
		src.format(
			fastJsonStableStringify(
				src.getFileInfo.sync(context.argv["file-info"], options)
			),
			{
				parser: "json",
			}
...
function getOptionsOrDie(context, filePath) {
	try {
		...
		context.logger.debug(
			"loaded options `" + JSON.stringify(options) + "`"
		);
		return options;
	} catch (error) {
...
function formatStdin(context) {
	...
	const relativeFilepath = context.argv["ignore-path"]
		? path__default["default"].relative(
				path__default["default"].dirname(context.argv["ignore-path"]),
				filepath
		  )
		: path__default["default"].relative(process.cwd(), filepath);
	getStream(process.stdin)
		.then((input) => {
			if (
				relativeFilepath &&
				ignorer.ignores(fixWindowsSlashes$1(relativeFilepath))
			) {
				writeOutput(context, {
					formatted: input,
				});
...
function formatFiles(context) {
	...
	for (const pathOrError of expandPatterns$1(context)) {
		...
		const ignoreFilename = context.argv["ignore-path"]
			...
			: filename;
		const fileIgnored = ignorer.ignores(
			fixWindowsSlashes$1(ignoreFilename)
		);
		...
		if (context.argv.write) {
			...
			if (isDifferent) {
				...
				context.logger.log(
					`${source.grey(filename)} ${Date.now() - start}ms`
				);
			}
...
function createUsage(context) {
	...
	const restCategories = Object.keys(groupedOptions).filter(
		(category) => !constant$1.categoryOrder.includes(category)
	);
	const allCategories = [
		...firstCategories,
		...restCategories,
...
function createLogger(logLevel) {
	...
	function createLogFunc(loggerName, color) {
		...
		return function (message, opts) {
			...
			stream.write(
				message.replace(/^/gm, prefix) + (opts.newline ? "\n" : "")
			);
		};
	}
...
function normalizeDetailedOptionMap(detailedOptionMap) {
	return fromPairs_1(
		Object.entries(detailedOptionMap)
			...
			.map(([name, option]) => [
				name,
				normalizeDetailedOption(name, option),
			])
	);
}

function createMinimistOptions(detailedOptions) {
	const [boolean, string] = partition_1(
		detailedOptions,
		({ type }) => type === "boolean"
	).map((detailedOptions) =>
		flatten_1(
			detailedOptions.map(({ name, alias }) =>
				alias ? [name, alias] : [name]
			)
		)
	);
	const defaults = fromPairs_1(
		detailedOptions
			.filter(
				(option) =>
					!option.deprecated &&
...
function updateContextOptions(context, plugins, pluginSearchDirs) {
	const { options: supportOptions, languages } = src.getSupportInfo({
		showDeprecated: true,
		showUnreleased: true,
		showInternal: true,
		plugins,
...
function normalizeContextArgv(context, keys) {
	const detailedOptions = !keys
		? context.detailedOptions
		: context.detailedOptions.filter((option) =>
				keys.includes(option.name)
		  );
	const argv = !keys ? context.argv : pick_1(context.argv, keys);
	context.argv = optionsNormalizer.normalizeCliOptions(
		argv,
		detailedOptions,
		{
			logger: context.logger,
...
function run(args) {
	...
	try {
		...
		context.logger.debug(
			`normalized argv: ${JSON.stringify(context.argv)}`
		);
		...
		if (context.argv.help !== undefined) {
			context.logger.log(
				typeof context.argv.help === "string" &&
					context.argv.help !== ""
					? util$1.createDetailedUsage(context, context.argv.help)
					: util$1.createUsage(context)
			);
			process.exit(0);
		}
		...
		if (context.argv["support-info"]) {
			context.logger.log(
				src.format(fastJsonStableStringify(src.getSupportInfo()), {
					parser: "json",
				})
			);
			process.exit(0);
